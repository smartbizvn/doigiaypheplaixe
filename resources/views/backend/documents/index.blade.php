...
@section('title_page', 'Quản lý Văn bản')
                        ...
                        <table class="table align-middle nowrap_header_mobile table-bordered table-hover">
                            <thead class="text-muted table-light">
                                <tr class="text-uppercase">
                                    ...
                                    <th class="text-left" width="220">Số ký hiệu</th>
                                    <th class="text-left">Trích yếu</th>
                                    <th class="text-left" width="200">Thông tin văn bản</th>
                                    <th class="text-center" width="150">Ngày ban hành</th>
                                    {{-- Cột chung --}}
                                ...
                                @if (!empty($results) && $results->count() > 0)
                                    @foreach ($results as $index => $result)
                                        <tr>
                                            ...
                                            <td>
                                                <a target="_blank" title='Sửa' href="{{ url('he-thong-van-ban', $result->id) }}">
                                                    {{ $result->epitome ?? "" }}
                                                </a>
                                            </td>
                                            <td class='text-left'>
                                                {!! collect([
                                                    $result->classInfo->name ?? null,
                                                    $result->agencyInfo->name ?? null,
                                                    $result->fieldInfo->name ?? null,
                                                    $result->typeInfo->name ?? null,
                                                ])->filter()->implode('<br>') !!}
                                            </td>
                                            <td class='text-center'>
                                                {{ $result->date_issue ?? "" }}
