        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header align-items-center d-flex">
                    <h4 class="card-title mb-0 flex-grow-1">Thông tin vai trò</h4>
                </div>
                <div class="card-body p-3 p-sm-4">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-lg-9">
                            <select class="form-control select2 select2-multiple" name="permissions[]" multiple="multiple" data-placeholder="Chọn quyền">
                                @if (!empty($permissions))
                                    @foreach($permissions as $permission)
                                        <option value="{{ $permission->id }}" {{ (in_array($permission->id, old('permissions', [])) || $result->permissions->contains($permission->id)) ? 'selected' : '' }}>{{ $permission->name }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
